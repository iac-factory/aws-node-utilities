...
import { Prompt, Handler } from "./src";
...
( async () => {
    ...
    const { Token } = await Prompt([ {
        prompt: "GitHub Personal-Access-Token (GH_TOKEN)" + ":" + " ", key: "Token"
    } ], { normalize: false });
    ...
    const scope = Package.name.split("/")[ 0 ];
    ...
    Subprocess.spawnSync("npx", [ "--yes", "lerna@latest", "publish", "--yes", "--registry", "https://npm.pkg.github.com", "from-git", "--canary", "--no-private" ], {
        shell: false,
        env: process.env,
        cwd: process.cwd(),
        stdio: "inherit",
